...
Python 3.11 / PyTorch 2.x safe version
"""
import os
import math
import numpy as np
...
import collections.abc  # Safe for Python 3.11
...
def get_criterion(p):
    if p['criterion'] == 'simclr':
        ...
        criterion = ConfidenceBasedCE(p['confidence_threshold'],
                                      p['criterion_kwargs']['apply_class_balancing'])
    else:
        raise ValueError(f'Invalid criterion {p["criterion"]}')
...
def get_model(p, pretrain_path=None):
    ...
    if p['backbone'] == 'resnet18':
        ...
        raise ValueError(f'Invalid backbone {p["backbone"]}')
    ...
    if p['setup'] in ['simclr', 'moco']:
        ...
        raise ValueError(f'Invalid setup {p["setup"]}')

    # Load pretrained weights
    if pretrain_path is not None:
        if not os.path.exists(pretrain_path):
            raise ValueError(f'Path with pre-trained weights does not exist {pretrain_path}')
        state = torch.load(pretrain_path, map_location='cpu')

        if p['setup'] == 'scan':  # Transfer from contrastive
            ...
            assert set(missing[1]) in expected_missing
        elif p['setup'] == 'selflabel':  # Transfer from scan
            model_state = state['model']
            all_heads = [k for k in model_state.keys() if 'cluster_head' in k]
            best_head_weight = model_state[f'cluster_head.{state["head"]}.weight']
            best_head_bias = model_state[f'cluster_head.{state["head"]}.bias']
            for k in all_heads:
                model_state.pop(k)
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    if p['train_db_name'] == 'cifar-10':
        ...
        subset_file = f'./data/imagenet_subsets/{p["train_db_name"]}.txt'
        dataset = ImageNetSubset(subset_file=subset_file, split='train', transform=transform)
    else:
        raise ValueError(f'Invalid train dataset {p["train_db_name"]}')

    # Wrap dataset for augmented or nearest neighbor use
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    if p['val_db_name'] == 'cifar-10':
        ...
        from data.imagenet import ImageNetSubset
        subset_file = f'./data/imagenet_subsets/{p["val_db_name"]}.txt'
        dataset = ImageNetSubset(subset_file=subset_file, split='val', transform=transform)
    else:
        raise ValueError(f'Invalid validation dataset {p["val_db_name"]}')
...
def get_train_transformations(p):
    if p['augmentation_strategy'] == 'standard':
        ...
        return transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize(**p['augmentation_kwargs']['normalize']),
            Cutout(**p['augmentation_kwargs']['cutout_kwargs'])
        ])
    else:
        raise ValueError(f'Invalid augmentation strategy {p["augmentation_strategy"]}')
...
def get_optimizer(p, model, cluster_head_only=False):
    if cluster_head_only:
        for name, param in model.named_parameters():
            param.requires_grad = 'cluster_head' in name
    ...
    if p['optimizer'] == 'sgd':
        ...
        raise ValueError(f'Invalid optimizer {p["optimizer"]}')
...
def adjust_learning_rate(p, optimizer, epoch):
    ...
    if p['scheduler'] == 'cosine':
        ...
        if steps > 0:
            lr *= (p['scheduler_kwargs']['lr_decay_rate'] ** steps)
    elif p['scheduler'] == 'constant':
        pass
    else:
        raise ValueError(f'Invalid learning rate schedule {p["scheduler"]}')

...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset.lower() != "imagenet":
        if args.num_labels == 10 and args.dataset == 'cifar10':
            ...
            fixmatch_index = [11530, 17059, 38649, 15281, 31729, 31501, 29779, 4244, 2042, 46412]
            #index = fixmatch_index[-args.seed - 1]
            index = np.array(fixmatch_index)
            print("10 labels for cifar10")
        elif args.num_labels == 30 and args.dataset == 'cifar10':
            fixmatch_index = [18668, 32109, 42315, 9100, 25931, 38739, 16995, 3682, 31677, 34580, 17558, 11634, 32286, 24602, 45266, 23744, 38224, 31018, 19459, 38978, 16468, 2814, 42135, 28499, 44157, 49558, 8133, 33683, 5666, 26282]
            index = np.array(fixmatch_index)
            print("30 labels for cifar10")
        elif args.num_labels == 60 and args.dataset == 'cifar10':
            fixmatch_index = [16255, 4592, 38224, 19019, 49081, 34580, 9064, 17410, 31679, 16868, 3682, 23744, 3498, 24602, 34290, 8254, 8133, 42528, 32109, 11634, 7636, 2814, 19459, 5504, 17558, 4565, 7349, 30892, 21577, 17063, 5666, 33683, 
25374, 35407, 26513, 2693, 4161, 25931, 38927, 30473, 42135, 1277, 11332, 49558, 45266, 31018, 16995, 23208, 44994, 46075, 47136, 26282, 38739, 7570, 38978, 16506, 28499, 16468, 18668, 36130]
            index = np.array(fixmatch_index)
            print("60 labels for cifar10")
        else:
            index = None
